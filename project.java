...
class CV_Fincorp extends User
{
	...
	static void collectLoanDetails(int principle,int index)
	{
		...
		if (loan_specify.equals("yes") || loan_specify.equals("Yes")) 
		{
			...
			if (p <= principle) 
			{
				...
				System.out.println(GREEN + "  Yes, we as a CV Fincorp can sanction you a loan of Rs." + p + ".00/-" + RESET);
				System.out.println();
				...
				switch (o) 
				{
					...
					case 3: 
					{
						...
						int ti = Validations.months_valid();
						details.get(index).time = ti;
						System.out.println(YELLOW + "  You have selected Equated Monthly Installment(EMI) for your Loan" + RESET);
						obj.loanDetailsDisplay(p, ul);
						float t = ((float) ti) / 12;
					...
					case 4: 
					{
						...
						if (loan_specify.equals("yes") || loan_specify.equals("Yes")) 
						{
							...
							details.get(index).loanID = ul.getLoanID();
							System.out.println(GREEN + "  Your Loan is sanctioned." + RESET);
							System.out.println(CYAN + "  Do you need CDI EMI for any particular month? please specify yes/no: "+ RESET);
    ...
    public static void main(String[]args)
	{
		...
		System.out.print(ORG+BLINK+"    CV FINCORP    "+RESET);
		System.out.println(CYAN+"*"+RESET);
		System.out.println(CYAN+"                       ********************"+RESET);
		...
		while(b)
		{
			...
			if(option.equals("2"))   //registration
			{
                ...
                else if(option1.equals("2"))  //fd register
				{
					...
					System.out.println(GREEN+"  		You are eligible for Registration Process."+RESET);
					System.out.print(BRIGHT_PINK + "	Welcome to CV Fincorp FD Registration Process	\n" + RESET);
					System.out.println();
			...
			else if(option.equals("1"))   //login
			{
                ...
                else if(option1.equals("2")) //FD Login
				{
						...
						if (details2.get(i).userID.equals(id)) 
						{
							...
							if (details2.get(i).password.equals(pass_word)) 
							{
								...
								if (details2.get(i).deposit_holder.equals("no")|| details2.get(i).deposit_holder.equals("No")) 
								{
									...
									System.out.println();
									System.out.print(CYAN+ "  Do you want to Deposit your amount in CV Fincorp please specify yes/no?"+ RESET);
					...
					if (c == 0) 
					{
						System.out.println(GREEN + "  You are not a registered person." + RESET);
					}
				}
				else  if(option1.equals("3"))
				{
...
class User_details extends CV_Fincorp
{
    ...
    void collectDetails()
	{
		...
		for(int i=0;i<=details.size()-1;i++)
		{
			if (details.get(i).phno.equals(phno) || details.get(i).aadhar == aadhar || details.get(i).pan.equals(pan) || details.get(i).emailID.equals(emailID)) 
			{
	...
	{
		System.out.println();
		System.out.println(BRIGHT_PINK + "    		Welcome " + this.name);
		System.out.println("");
...
class User_details2 extends CV_Fincorp
{
    ...
    void collectDetails2(User_details2 obj)
	{
		...
		for(int i=0;i<=details2.size()-1;i++)
		{
			if (details2.get(i).phno.equals(phno) || details2.get(i).aadhar == aadhar || details2.get(i).pan.equals(pan) || details2.get(i).emailID.equals(emailID)) 
			{
				System.out.println("You are already registered person in Loan process.");
				unique = false;
				break;
		...
		{
			System.out.println("Atleast one of the four details namely aadhar, pan, phone. no.,email address are already registered person in Loan process.Try with other details");
			this.collectDetails2(obj);
		}
    }
	...
	static String gender_validation()
	{
		...
		while(b)
		{
			...
			if(b)
			{
				...
				else if(gender.equals("O") || gender.equals("Other") || gender.equals("other") || gender.equals("o"))
				{
					gender = "Other";
				}
			}
	...
	static String getuserID()
	{
		...
		{  
  			...
  			else
			{
				if(c==3)
				{
					System.out.println(PURPLE+"  You are reached maximum attempts"+RESET);
					System.out.println();
					count+=c;
					return count;
				}
